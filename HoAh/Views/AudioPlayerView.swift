    ...
    var body: some View {
        Capsule()
            .fill(
                LinearGradient(
                    colors: [
                        isPlayed ? theme.accentColor : theme.accentColor.opacity(0.3),
                        isPlayed ? theme.accentColor.opacity(0.8) : theme.accentColor.opacity(0.2)
                    ],
                    startPoint: .bottom,
                    endPoint: .top
                )
...
struct AudioPlayerView: View {
                ...
                if showRetranscribeSuccess {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8)
                            .fill(theme.statusSuccess.opacity(0.1))
                            .stroke(theme.statusSuccess.opacity(0.2), lineWidth: 1)
                    )
                ...
                if showRetranscribeError {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8)
                            ...
                            .stroke(theme.statusError.opacity(0.2), lineWidth: 1)
                    )
                    .transition(.move(edge: .top).combined(with: .opacity))
                }
    ...
    private func retranscribeAudio() {
        guard let currentTranscriptionModel = whisperState.currentTranscriptionModel else {
            errorMessage = "No dictation model selected"
