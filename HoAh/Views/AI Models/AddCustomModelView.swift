            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text("Only OpenAI-compatible dictation APIs are supported")
                            .font(theme.typography.caption)
                            .foregroundColor(theme.textSecondary)
                    }
                        ...
                        Button(action: {
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 8)
                                    .fill(isFormValid ? theme.accentColor : theme.textMuted)
                                    .shadow(color: theme.shadowColor.opacity(0.2), radius: 2, x: 0, y: 1)
                            )
                        }
                        .buttonStyle(.plain)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if let editing = editingModel {
                ...
                let updatedModel = CustomCloudModel(
                    ...
                    name: generatedName,
                    displayName: trimmedDisplayName,
                    description: "Custom dictation model",
                    apiEndpoint: trimmedApiEndpoint,
                    apiKey: trimmedApiKey,
                    modelName: trimmedModelName,
