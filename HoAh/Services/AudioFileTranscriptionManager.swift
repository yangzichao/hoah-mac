...
class AudioTranscriptionManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.yangzichao.hoah", category: "AudioTranscriptionManager")
    ...
    func startProcessing(url: URL, modelContext: ModelContext, whisperState: WhisperState) {
        ...
        currentTask = Task {
            do {
                ...
                guard let localTranscriptionService else {
                    throw TranscriptionError.localTranscriptionUnavailable
                }
                ...
                var text: String
                var effectiveModel: any TranscriptionModel = currentModel
                ...
                do {
                    ...
                    if let fallbackModel = whisperState.offlineFallbackLocalModel(for: currentModel, error: error) {
                        logger.warning(
                            "Imported audio transcription hit connectivity issue. Falling back to local model \(fallbackModel.name, privacy: .public)"
                        )
                        ...
                        text = try await localTranscriptionService.transcribe(audioURL: permanentURL, model: fallbackModel)
                    } else {
                        throw error
                    }
                }
                ...
                if let enhancementService = whisperState.enhancementService,
                   ...
                   enhancementService.isConfigured {
                    ...
                    do {
                        ...
                        logger.error("AI Action failed: \(error.localizedDescription)")
                        let transcription = Transcription(
                            text: text,
                            duration: duration,
                            audioFileURL: permanentURL.absoluteString,
...
enum TranscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .localTranscriptionUnavailable:
            return "Local dictation service is unavailable"
        case .transcriptionCancelled:
