...
class CustomModelManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.yangzichao.hoah", category: "CustomModelManager")
    ...
    func removeCustomModel(withId id: UUID) {
        if let model = customModels.first(where: { $0.id == id }) {
            model.deleteApiKey()
        }
    ...
    private func loadCustomModels() {
        ...
        do {
            ...
            // Rewrite once so any legacy plaintext apiKey payload is removed from persisted JSON.
            saveCustomModels()
        } catch {
