...
final class OpenAITranscriptionService {
    private let logger = Logger(subsystem: "com.yangzichao.hoah", category: "OpenAITranscriptionService")
    ...
    func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            logger.error("OpenAI transcription request failed with status \(httpResponse.statusCode): \(errorMessage, privacy: .public)")
        ...
        do {
            ...
            logger.error("Failed to decode OpenAI transcription response: \(error.localizedDescription)")
    ...
    private func getAPIConfig(for model: any TranscriptionModel) throws -> APIConfig {
        ...
        guard let apiURL = URL(string: "https://api.openai.com/v1/audio/transcriptions") else {
            throw NSError(domain: "OpenAITranscriptionService", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid API URL"])
        }
