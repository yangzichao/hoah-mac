...
class WhisperPrompt: ObservableObject {
    @Published var transcriptionPrompt: String = UserDefaults.hoah.string(forKey: "TranscriptionPrompt") ?? ""
    ...
    private func loadCustomPrompts() {
        if let savedPrompts = UserDefaults.hoah.dictionary(forKey: customPromptsKey) as? [String: String] {
            customPrompts = savedPrompts
        }
    }
    
    private func saveCustomPrompts() {
        UserDefaults.hoah.set(customPrompts, forKey: customPromptsKey)
        UserDefaults.hoah.synchronize() // Force immediate synchronization
    }
    ...
    func updateTranscriptionPrompt() {
        ...
        UserDefaults.hoah.set(prompt, forKey: "TranscriptionPrompt")
        UserDefaults.hoah.synchronize() // Force immediate synchronization
